...
class ASRModel(torch.nn.Module):
    ...
    def forward(
        ...
        if self.ctc_weight != 0.0:
            loss_ctc = self.ctc(encoder_out, encoder_out_lens, targets,
                                targets_length)
        else:
            loss_ctc = None
    ...
    def recognize(
        ...
        for i in range(1, maxlen + 1):
            ...
            # logp: (B*N, vocab)
            logp, cache,_ = self.decoder.forward_one_step(
    ...
    def attention_rescoring(
        ...
        r_hyps_pad, _ = add_sos_eos(r_hyps_pad, self.sos, self.eos,
                                    self.ignore_id)
        decoder_out, r_decoder_out, pre_decoder_output = self.decoder(
        ...
        for i, hyp in enumerate(hyps):
            ...
            if score > best_score:
                best_score = score
                best_index = i
        return hyps[best_index][0], encoder_out, pre_decoder_output
